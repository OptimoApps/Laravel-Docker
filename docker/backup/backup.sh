...
copy_s3() {
  ...
  cat $SRC_FILE | aws $AWS_ARGS s3 cp - s3://$S3_BUCKET/$DATABASE_DIR/$DEST_FILE
...
MONGODUMP_HOST_OPTIONS="--host ${MONGODB_HOST} --port ${MONGODB_PORT} -u ${MONGODB_USER} -p ${MONGODB_PASSWORD} --authenticationDatabase admin"
